    ...
    override func spec() {
        ...
        describe("Repository.fetch(_:)") {
            it("should fetch the data") {
                ...
                expect(repo.fetch(remote.name).value).toNot(beNil())
            }
        }
        ...
        describe("Repository.status") {
            ...
            it("Should accurately report status for repositories with status") {
                ...
                var unstagedFilePaths: [String] = []
                for status in statuses {
                    if status.status == .workTreeNew {
                        unstagedFilePaths.append((status.indexToWorkDir?.newFile?.path)!)
                    } else {
                ...
                expect(unstagedFilePaths).to(equal(expectedUnstagedPaths))
            }
        }
